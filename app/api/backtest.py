...
def run_backtest(request: BacktestRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Error fetching market data: {str(e)}"
        )
    ...
    try:
        ...
        results = backtest_engine.run(
            strategy_type=strategy.strategy_type, strategy_params=strategy.parameters, data=data
        )
    except Exception as e:
        raise HTTPException(
...
def list_backtests(
    strategy_id: int = None, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)
):
    """
